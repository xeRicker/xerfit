...
import { Product, MealEntry, UserProfile, Measurement } from './store';
...
export async function fetchAllData() {
  try {
    ...
    const entries = results[2] as DbRow[];
    const measurements = results[3] as DbRow[];
    const settings = results[4] as DbRow[];
