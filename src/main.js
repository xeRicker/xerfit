...
const LOCAL_PROFILES_KEY = 'db/profile-list';
...
class App {
    ...
    getLocalProfiles() {
        const fromLs = localStorage.getItem(LOCAL_PROFILES_KEY);
        if (fromLs) return JSON.parse(fromLs);
        return [{ id: 'default', name: 'Domyślny', avatar: 'profile1' }];
    }

    async loadProfiles() {
        const remote = await GitHubDataService.loadProfiles();
        const profiles = remote?.length ? remote : this.getLocalProfiles();
