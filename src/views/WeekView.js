...
export class WeekView extends Component {
    ...
    update(state) {
            ...
            <div style="padding: 0 16px;">
                <div class="card" style="margin: 0 0 12px 0;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 12px;">
                        ...
                        <strong>${weekDays[0].toLocaleDateString('pl-PL')} - ${weekDays[6].toLocaleDateString('pl-PL')}</strong>
                    ...
                    <div style="display:grid; grid-template-columns: repeat(7, 1fr); gap:6px;">
                        ${dayStats.map(day => {
                            ...
                            <button class="week-day ${active ? 'active' : ''}" data-date="${day.key}" style="padding:10px 6px; border-radius: 12px; border:1px solid var(--line-soft); background:${active ? 'rgba(93,233,255,.16)' : 'rgba(255,255,255,.02)'}; color:var(--text-main);">
                                <div style="font-size:11px; color:var(--text-sub);">${new Date(day.key).toLocaleDateString('pl-PL', { weekday: 'short' })}</div>
                ...
                <div class="card" style="margin: 0 0 16px 0;">
                    <h3 style="margin-bottom:10px;">${new Date(selected.key).toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long' })}</h3>
        ...
        this.container.querySelector('#week-prev').onclick = () => { this.weekOffset -= 1; this.update(store.state); };
        this.container.querySelector('#week-next').onclick = () => { this.weekOffset += 1; this.update(store.state); };
        this.container.querySelectorAll('.week-day').forEach(btn => {
            btn.onclick = () => {
